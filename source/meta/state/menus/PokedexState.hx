...
class PokedexState extends MusicBeatState {
    ...
    override public function create() {
        ...
        for (i in folderList) {
            ...
            if (HSys.exists(Paths.getPath('images/pokedex/${i}/info.json', TEXT))) 
            {
                var rawJson = HSys.getContent(Paths.getPath('images/pokedex/${i}/info.json', TEXT));
        ...
        for (i in 0...dexArray.length)
            {
                ...
                if (HSys.exists(Paths.getPath('images/pokedex/'+ daPoke + '/info.json', TEXT))) {
                    var rawJson = HSys.getContent(Paths.getPath('images/pokedex/' + daPoke + '/info.json', TEXT));
                    var swagShit:PokeData = cast Json.parse(rawJson).info;
    ...
    override public function update(elapsed:Float) {
        ...
        var back:Bool = controls.BACK || HSys.androidBack();
