...
class ShopState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var rawJson = HSys.getContent(Paths.getPath('images/shop/shopText.json', TEXT)).trim();
		...
		for (i in folderList)
		{
			...
			if (HSys.exists(Paths.getPath('images/shop/${i}/${i}.json', TEXT)))
			{
				var rawJson = HSys.getContent(Paths.getPath('images/shop/${i}/${i}.json', TEXT));
	...
	function addToFreeplayList(i:String, ?library:String)
	{
		for (j in 0...2)
		{
			...
			var icon:String = 'gf';
			var chartExists:Bool = HSys.exists(Paths.songJson(i, i + '-hard', old, library));
			if (library != null)
				chartExists = HSys.exists(Paths.songJson(i, i + '-hard', old, library), TEXT);
			if (chartExists)
			{
	...
	override public function update(elapsed:Float)
	{
		...
		if (freeplaySelected)
		{
			if (canControl)
			{
				...
				if (controls.ACCEPT)
				{
					...
					if (songs[verticalSelection].songName.toLowerCase() == 'death-toll' && mechDiff == 'pussy')
					{
						FlxG.sound.play(Paths.sound('errorMenu'));
						camera.shake(0.005, 0.06);
					}
					else if (FlxG.save.data.unlockedSongs.contains(songs[verticalSelection].songName.toLowerCase()))
					{
						canControl = false;
						endingFreeplay = true;
		...
		if (controls.BACK || HSys.androidBack() && !inCutscene && !endingFreeplay)
		{
			if (inShop)
			{
				inShop = false;
				...
				while (true)
				{
					...
					if (!killThread)
					{
						...
						if (portrait != null && portrait != curPortrait)
						{
							...
							var esewey:String = 'menus/freeplay/$portrait';
							portrait = '$portrait';
							...
							portraitGraphic = Paths.getSparrowAtlas(esewey.toLowerCase());
							portraitLoaded = true;
							curPortrait = portrait;
							mutex.release();
