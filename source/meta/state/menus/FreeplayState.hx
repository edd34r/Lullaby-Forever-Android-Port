...
class FreeplayState extends MusicBeatState
{
		...
		for (i in folderSongs)
		{
			if (!existingSongs.contains(i.toLowerCase()))
			{
				for (j in 0...2) {
					...
					var icon:String = 'gf';
					var chartExists:Bool = HSys.exists(Paths.songJson(i, i, old));
					if (chartExists)
					{
	...
	public function addSong(songName:String, weekNum:Int, songCharacter:String, ?library:String, old:Bool, songColor:FlxColor)
	{
		...
		for (i in CoolUtil.difficultyArray)
			if (HSys.exists(Paths.songJson(songName, songName + '-' + i, old))
				|| (HSys.exists(Paths.songJson(songName, songName, old)) && i == "NORMAL"))
