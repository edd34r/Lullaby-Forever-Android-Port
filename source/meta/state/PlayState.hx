...
class PlayState extends MusicBeatState
{
	...
	public static var botplayQuotes:Map<String, Array<String>> = [
		...
		'missingno' => ['mew was under the truck', 'i hope your mobile (bru, "pc") actually crashes'],
		'brimstone' => [
			'brimstone betadciu tyfpe beat',
	...
	var vpad:android.FlxVirtualPad;
	#end

	public var pressSpace:Bool = false;
	...
	override public function create()
	{
		...
		if (HSys.exists(Paths.songJson(SONG.song.toLowerCase(), 'lyrics', false))) {
			trace('ly rics');
		...
		#if MOBILE_CONTROLS // !1!!1!!
		if (gameplayMode != PUSSY_MODE)
		{
			Main.isBronzong = SONG.song.toLowerCase() == 'death-toll';
			Main.isHypno = SONG.song.toLowerCase() == 'frostbite' || SONG.player2 == 'hypno' || SONG.player2 == 'hypno-two' || SONG.player2 == 'abomination-hypno' || tranceActive || ((playerLane == 0 || playerLane == 1) && SONG.song == "Pasta-Night");
			if (Init.trueSettings.get('Hitbox Type') == 'Virtual Pad')
			{
				Main.isHypno = false;

				if (SONG.song.toLowerCase() == 'death-toll' || SONG.song.toLowerCase() == 'frostbite' || SONG.player2 == 'hypno' || SONG.player2 == 'hypno-two' || SONG.player2 == 'abomination-hypno' || tranceActive || ((playerLane == 0 || playerLane == 1) && SONG.song == "Pasta-Night"))
				{
					Main.isVPad = true;
					Main.isBronzong = false;
				}
				else
					Main.isVPad = false;
			}
		...
		switch (mControls.mode)
		{
			case VIRTUALPAD_RIGHT | VIRTUALPAD_LEFT | VIRTUALPAD_CUSTOM:
				...
				if (gameplayMode != PUSSY_MODE)
				{
					Main.isVPad = SONG.song.toLowerCase() == 'death-toll' || SONG.song.toLowerCase() == 'frostbite' || SONG.player2 == 'hypno' || SONG.player2 == 'hypno-two' || SONG.player2 == 'abomination-hypno' || tranceActive || ((playerLane == 0 || playerLane == 1) && SONG.song == "Pasta-Night");
					Main.isHypno = false;
					Main.isBronzong = false;
				}
			case HITBOX:
		...
		if (Main.isVPad) vpad.cameras = [camControl];
	...
	function set_snowIntensity(value:Float):Float
	{
		if (canChangeIntensity) {
			snowIntensity = value;
			trace(value);
			if (frostbiteShader != null) frostbiteShader.shader.data.intensity.value = [snowIntensity];
		}
	...
	function set_snowAmount(value:Float):Float
	{
		if (canChangeAmount)
		{
			snowAmount = value;
			trace(value);
			if (frostbiteShader != null) frostbiteShader.shader.data.amount.value = [Std.int(snowAmount)];
		}
		return snowAmount;
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'frostbite' && useFrostbiteMechanic) // bool in hxs script
		{
			if (getMobileSpace('justPressed'))
			{
				pressSpace = true;
		...
		if (dialogueBox != null && dialogueBox.alive) {
			...
			if (FlxG.keys.justPressed.SHIFT || HSys.androidBack())
				dialogueBox.closeDialog();

			#if MOBILE_CONTROLS
			if (HSys.justTouched() && dialogueBox.textStarted)
			#else
		...
		if (!inCutscene && generatedMusic) {
			...
			if (FlxG.keys.justPressed.ENTER || HSys.androidBack() && startedCountdown && (accuracySound == null || (accuracySound != null && !accuracySound.playing)) && canPause && !deadstone)
			{
			...
			strumLines.members[playerLane].autoplay = Init.trueSettings.get('Autoplay');
			...
			if (!deadstone) {
				...
				else
				{
					...
					if (pendulum != null && tranceActive) {
						...
						if (!tranceNotActiveYet) {
							...
							// /*
							if (getMobileSpace('justPressed') || (strumLines.members[playerLane].autoplay && canHitPendulum && !alreadyHit))
							{
								if (canHitPendulum)
								{
									canHitPendulum = false;
				...
				if ((useFrostbiteMechanic && typhlosionUses >= 1 && typhlosion.animation.curAnim.name != 'fire' && gameplayMode != PUSSY_MODE) && (strumLines.members[playerLane].autoplay && coldness >= 0.5 || (getMobileSpace('justPressed') && !strumLines.members[playerLane].autoplay)))
					{
						useTyphlosion();
					}
	...
	private function mainControls(daNote:Note, char:Array<Character>, strumline:Strumline, autoplay:Bool):Void
	{
		...
		var holdControls:Array<Bool> = [controls.LEFT, controls.DOWN, controls.UP, controls.RIGHT, getMobileSpace('pressed')];
		if (!autoplay) {
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (HSys.exists(Paths.songJson(SONG.song.toLowerCase(), 'events', old))) {
			trace('events found');
	...
	private function callDefaultSongEnd(videoName:String = '')
	{
		...
		else 
		{
			...
			if (videoName == '')
				...
				new WebviewHandler(videoName).finishCallback = function() 
				{
					FlxG.switchState(new PlayState());
				}
			}
	...
	public function startCountdown():Void
	{
		...
		switch (SONG.song.toLowerCase())
		{
			case 'frostbite' | 'dissension' | 'insomnia' |
				'isotope' | 'dissension' | 'bygone-purpose' | 'shitno':
				camHUD.alpha = 0;
				uiHUD.alpha = 0;
				for (i in strumHUD)
					i.alpha = 0;
	...
	#if MOBILE_CONTROLS // null function
	public function getMobileSpace(action:String = 'null'):Bool
	{
		if (Main.isHypno || Main.isBronzong)
		{
			switch (action)
			{
				case 'pressed':
					return mControls.hitbox.buttonSpace.pressed || FlxG.keys.pressed.SPACE;
				case 'justPressed':
					return mControls.hitbox.buttonSpace.justPressed || FlxG.keys.justPressed.SPACE;
				case 'justReleased':
					return mControls.hitbox.buttonSpace.justReleased || FlxG.keys.justReleased.SPACE;
				default:
					return false;
					trace('action null, dum');
			}
		}
		else if (Main.isVPad)
		{
			...
			{
				case 'pressed':
					return vpad.buttonA.pressed || FlxG.keys.pressed.SPACE;
				case 'justPressed':
					return vpad.buttonA.justPressed || FlxG.keys.justPressed.SPACE;
				case 'justReleased':
					return vpad.buttonA.justReleased || FlxG.keys.justReleased.SPACE;
				default:
					return false;
					trace('action null, dum');
			}
