...
class Events {
	...
	public static function obtainEvents() { // without Assets.list()
		loadedModules.clear();
		eventList = [];
		...
		for (event in tempEventArray) {
			if (event.contains('.')) {
				...
				loadedModules.set(event, ScriptHandler.loadModule('assets/events/$event'));
				...
				if (PlayState.SONG != null && CoolUtil.spaceToDash(PlayState.SONG.song.toLowerCase()) == event) {
					...
					for (subEvent in internalEvents)
					{
						...
						loadedModules.set(subEvent, ScriptHandler.loadModule('assets/events/$event/$subEvent'));
